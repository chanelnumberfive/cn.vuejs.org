...
这只是一个假设的例子，但是我们已经设法将应用分割成了两个更小的单元，子单元通过 `props` 接口实现了与父单元很好的解耦。我们现在可以进一步为我们的 `todo-item` 组件实现更复杂的模板和逻辑的改进，而不会影响到父单元(示例中的v-bind:后跟的todo不支持破折号，也不支持驼峰命名，使用后均报错，好像就支持小写，只是个人觉得支持破折号是不是更易阅读而且与html语法表现一致？)。
